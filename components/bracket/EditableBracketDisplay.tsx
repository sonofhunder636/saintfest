...
import { useBracketDimensions, useTextWrapping } from '@/hooks/useBracketDimensions';
...
export default function EditableBracketDisplay({
  ...
  const { config, connections, totalWidth, totalHeight, isCalculating } = useBracketDimensions(bracket);
        ...
        <div className="relative" style={{ width: `${totalWidth}px`, height: `${totalHeight}px` }}>
          ...
          {bracket.rounds.map(round => 
            round.matches.map(match => {
              const position = config.positions[match.matchId];
...
function EditableMatchDisplay({ 
  ...
  const { lines: lines1 } = useTextWrapping(name1, position.lineLength - 80);
  const { lines: lines2 } = useTextWrapping(name2, position.lineLength - 80);
