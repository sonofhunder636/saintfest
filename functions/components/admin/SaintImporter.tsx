...
export default function SaintImporter() {
  const { adminPost, isAdminAuthenticated } = useAdminAPI();
  ...
  const handleImportSaints = async () => {
    if (!isAdminAuthenticated) {
      setImportStatus({
        type: 'error',
        message: 'Admin authentication required. Please ensure you are logged in as an admin.',
      });
      return;
    }
    ...
    try {
      const response = await adminPost('/api/admin/import-saints', { googleSheetsUrl });
      ...
      <CardContent className="space-y-4">
        ...
        {!isAdminAuthenticated && (
          <Alert>
            ...
            <AlertDescription>
              Admin authentication required. Please ensure you are logged in with admin privileges.
            </AlertDescription>
          </Alert>
        )}
        ...
        <Button
          ...
          disabled={importing || !googleSheetsUrl.trim() || !isAdminAuthenticated}
          className="w-full"
        >
          {importing ? (
            <>
